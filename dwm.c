...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		/* w = m->tagset[m->seltags] & 1 << i ? TEXTW("") : TEXTW(""); */
    w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeSel : SchemeNorm]);
		drw_text(drw, x, 0, w, bh, lrpad / 2, m->tagset[m->seltags] & 1 << i ? "" : (occ & 1 << i ? "" : tags[i]), urg & 1 << i);

    /* This draws the indictor that something is here */
		/* if (occ & 1 << i) */
